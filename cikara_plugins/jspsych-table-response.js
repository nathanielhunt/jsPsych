...
jsPsych.plugins["html-button-response"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    appendT = function (stim) {
        var $toAppend = $('<div/>', {
          id: "jspsych-html-button-response-stimulus"
        ...
        for (var i = 0; i < stim.length; i++) {
            ...
            for (var j = 0; j < stim[i].length; j++) {
                var clss = 'jspsych-html-button-response-button-' + i;
                $row.append($('<td/>', {
                  text: stim[i][j],
                  class: clss
    ...
    function after_response(choice) {
      ...
      $('.jspsych-html-button-response-stimulus').addClass('responded')
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function() {
        $('#jspsych-html-button-response-stimulus').css({'visibility': 'hidden'});
