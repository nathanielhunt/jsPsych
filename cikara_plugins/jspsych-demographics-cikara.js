...
jsPsych.plugins['demographics-cikara'] = (function() {
	...
	for (var i = 0; i < trial.questions.length; i++) {
	    ...
	    } else if (trial.input_type[i] == "radio") {
		// add radio box
		if (typeof trial.names != 'undefined') {
		    var name = trial.names[i];
		} else {
		...
		for(var j = 0; j < trial.choices[i].length; j++) {
		    ...
		    var $input = $('<input>', {
			type: 'radio',
			name: '#jspsych-radio' + name,
			id: j + 1
		    });

		    var $p = $('<p>', {
			html: trial.choices[i][j],
		    })
		...
		$table.append($tr);
		$("#jspsych-survey-text-" + i ).append($table);
	    }
	}
	...
	function validate( ) {
		...
		} else if ( trial.input_type[index] == "radio") {
		    var val = $('div#jspsych-survey-text-' + index + ' input:checked').attr('id');
		}
