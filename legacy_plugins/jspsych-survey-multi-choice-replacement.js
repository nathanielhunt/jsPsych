...
jsPsych.plugins['survey-multi-choice-replacement'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'survey-multi-choice-replacement',
    description: '',
    parameters: {
      questions: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    if (typeof replacements != 'undefined') {
      if (typeof replacements == 'object') {
        for (var k = 0, l = replacements.length; k < l; k ++) {
          var replacement = replacements[k];
          trial.options[i][j] = trial.options[i][j].replace(replacement[0], replacement[1]);
        }
      }
    }
	...
	$('.jspsych-survey-multi-choice-horizontal').css({'text-align': 'center'});
    }
	...
	$("div." + plugin_id_name + "-question").each(function(index) {
	    ...
	    var obje = {};
	    obje[id] = val;
	    obje[id + "_index"] = trial.options[index].indexOf(val);
            console.log(trial.options[index].indexOf(val));
            console.log(id + "_index");
            console.log(obje[id + "_index"]);
	...
	$.extend(trial_data, trial.arb_data);
